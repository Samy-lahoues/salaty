...
// Utility function to get all surah indices
export const getSurahIndices = (surahs: Surah[]): number[] => {
  return surahs.map((surah) => surah.id);
};

// Utility function to get surah by index
export const getSurahByIndex = (
  surahs: Surah[],
  index: number,
): Surah | undefined => {
  return surahs.find((surah) => surah.id === index);
};
export {
  navItems,
  GreenLogo,
